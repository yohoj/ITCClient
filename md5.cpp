...
//
void ToMD5(unsigned char digest[16],  void *input, unsigned int inputLen)
{
	MD5_CTX context;
	MD5Init (&context);
...
char *MD5Encode(const void *src, unsigned int len, char *secret) {
	int i;
	...
	MD5Final (digest, &context);
	for(i=0, secret[0]='\0'; i<16; i++)
		sprintf(secret+i+i, "%02x", digest[i]);
	secret[32] = 0;
	return secret;
}
